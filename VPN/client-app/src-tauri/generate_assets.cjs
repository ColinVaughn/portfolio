...
function createBmp(width, height, getPixelRGB, filePath) {
    ...
    for (let y = height - 1; y >= 0; y--) {
        for (let x = 0; x < width; x++) {
            const [r, g, b] = getPixelRGB(x, height - 1 - y);
...
createBmp(493, 312, (x, y) => {
    if (x < 164) {
        const r = Math.floor(bgColor[0] + (accentColor[0] - bgColor[0]) * y / 312);
        const g = Math.floor(bgColor[1] + (accentColor[1] - bgColor[1]) * y / 312);
        const b = Math.floor(bgColor[2] + (accentColor[2] - bgColor[2]) * y / 312);
        return [r, g, b];
    }
    return white;
